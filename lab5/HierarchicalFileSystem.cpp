...
int HierarchicalFileSystem::deleteFile(std::string path) {
	...
	if (dynamic_cast<DirectoryFile*>(fileToDelete) != nullptr) {
		if (fileToDelete->getSize() != 0) {
			cout << "unable to remove non-empty directory" << endl;
		...
		////cout << "not delete directory" << endl;
		if (openFiles.find(fileToDelete) != openFiles.end()) {
			//cout << "file is open" << endl;
			return fileisopen; // file in use
		}
		AbstractFile* parent = fileToDelete->getParent();
		//cout << "after get parent" << endl;
		if (parent == nullptr) {
			cout << "parent is nullptr" << endl;
		}
		parent->removeChild(fileToDelete->getName());
		//cout << "after remove child" << endl;
		/*if (dynamic_cast<ProxyFile*>(fileToDelete) != nullptr) {
			fileToDelete = (ProxyFile*)(fileToDelete);
			if (fileToDelete->getCounter() != 0) {
			}
		}
		else {
			delete fileToDelete;
		...
		delete fileToDelete;
		//cout << "after delete fileToDelete" << endl;
	}
	return success;
}
