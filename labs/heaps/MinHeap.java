...
//		while(loc!=1) {
//			if(loc%2 == 0) {
//				if(this.array[loc].getValue().compareTo(this.array[loc-1].getValue()) < 0) {
//					moveItem(loc, loc-1);
//					loc = loc - 1;
//				} else {
//					break;
...
//		}
	...
	private void heapify(int where) {
		//
		// As described in lecture
		if(2*where > this.size) {
			ticker.tick();
			return;
		}
		if(2*where + 1 > this.size) {
			if(this.array[2*where].getValue().compareTo(this.array[where].getValue())<0) {
				ticker.tick();
			...
			if(this.array[2*where].getValue().compareTo(this.array[2*where+1].getValue())<0) {
				if(this.array[2*where].getValue().compareTo(this.array[where].getValue())<0) {
					ticker.tick();
					moveItem(where,2*where);
					where = 2*where;
				...
				if(this.array[2*where+1].getValue().compareTo(this.array[where].getValue())<0) {
					ticker.tick();
					moveItem(where,2*where+1);
					where = 2*where+1;
					heapify(where);
				} 
			}
