...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while(pq.isEmpty() == false) {
    		...
    		for(Edge e: v.vertex.edgesFrom()) {
    			Decreaser<VertexAndDist> toHandle = handles.get(e.to);
    			VertexAndDist u = toHandle.getValue();
				if(v.distance + weights.get(e) < u.distance) {
    				toHandle.decrease(new VertexAndDist(u.vertex,v.distance + weights.get(e)));
    				parentEdges.replace(u.vertex, e);
    			}
    		}
    	}
    }
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	//
    	Vertex vc = endVertex;
    	while(parentEdges.get(vc).from != startVertex) {
    		path.addFirst(parentEdges.get(vc));
    		vc = parentEdges.get(vc).from;
    	}
    	path.addFirst(parentEdges.get(vc));
